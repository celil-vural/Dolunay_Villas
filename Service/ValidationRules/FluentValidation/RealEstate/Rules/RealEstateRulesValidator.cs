...
namespace Service.ValidationRules.FluentValidation.RealEstate.Rules
{
    public class RealEstateRulesValidator : AbstractValidator<RulesDto>
    {
        public RealEstateRulesValidator()
        {
            ...
            RuleFor(x => x.LocalIconId).Null().When(dto => !string.IsNullOrEmpty(dto.FontAwesomeIcon)).WithMessage("Fontawesome and LocalIcon must not be full at the same time."); RuleFor(x => x.CreatedByUser).NotEmpty().WithMessage("Create by user must not empty.Please log out of the system and log in again. ");
